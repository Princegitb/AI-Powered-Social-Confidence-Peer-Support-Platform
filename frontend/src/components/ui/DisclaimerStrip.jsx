...
/**
 * DisclaimerStrip — PRD Section 5.1
...
export default function DisclaimerStrip({ variant = 'chat' }) {
  if (variant === 'banner') {
    return (
      <div className="flex items-center gap-2.5 px-5 py-3 bg-surface-soft rounded-2xl text-[13px] text-text-secondary">
  ...
  return (
    <div className="flex items-center justify-center gap-1.5 px-4 py-2 bg-surface-soft/80 text-[11.5px] text-text-tertiary">
      ...
      <span>SAATHI is a support and practice tool, not a replacement for therapy or medical care.</span>
    </div>
  );
}
