...
async def _derive_level(user_id: str) -> dict:
    ...
    companion = await sessions_collection.count_documents({"user_id": user_id})
    roleplay = await roleplay_sessions_collection.count_documents({"user_id": user_id, "completed": True})
    peer = await peer_sessions_collection.count_documents({"user_id": user_id})
    challenges = await progress_logs_collection.count_documents({"user_id": user_id, "kind": "challenge_complete"})
...
async def advance_journey(req: AdvanceRequest):
